...
from .core import PrimitiveDB, SelectResult
    ...
    def execute(self, cmd: Command) -> bool:
        ...
        if cmd.name == "list_tables":
            ...
            for t in info:
                ...
                rows.append({"table": t["table"], "schema": cols, "file": t["rows_file"]})
        ...
        if cmd.name == "update":
            if not cmd.table or not cmd.set_values:
                raise ParseError("Синтаксис: update <table> set <field=value>,... [where ...]")
